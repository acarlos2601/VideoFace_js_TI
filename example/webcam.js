...
function drawFaces(canvas, data, fps) {
  ...
  for (const person of data) {
    ...
    const drawBox = new faceapi.draw.DrawBox(person.detection.box, { label: "face", boxColor: "yellow" })
    drawBox.draw(canvas)
    ...
    ctx.fillText(`gender ${Math.round(100 * person.genderProbability)}% ${person.gender}`, person.detection.box.x, person.detection.box.y + person.detection.box.height + 50);
    ctx.fillText(`expression ${Math.round(100 * expression[0][1])}% ${expression[0][0]}`, person.detection.box.x, person.detection.box.y + person.detection.box.height + 30);
    ctx.fillText(`age ${Math.round(person.age)} years`, person.detection.box.x, person.detection.box.y + person.detection.box.height + 10);
